...
const ALGORITHMS = {
  bubbleSort: {
    type: "array",
    build: (inputArr) => bubbleSortSteps(inputArr),
    code: bubbleSortCode,
  },
  selectionSort: {
    type: "array",
    build: (inputArr) => selectionSortSteps(inputArr),
    code: selectionSortCode,
  },
  insertionSort: {
    type: "array",
    build: (inputArr) => insertionSortSteps(inputArr),
    code: insertionSortCode,
  },
  bfs: {
    type: "graph",
    build: (g) => bfsSteps(g.graph, g.start),
    code: bfsCode,
  },
  dfs: {
    type: "graph",
    build: (g) => dfsSteps(g.graph, g.start),
    code: dfsCode,
  },
  knapsack01: {
    type: "dp",
    build: (d) => knapsack01Steps(d),
    code: knapsack01Code
  },

  mergeSort: {
    type: "array",
    build: (inputArr) => mergeSortSteps(inputArr),
    code: mergeSortCode,
  },
...
//   if (algorithm === "selectionSort") return [...inputArr].sort((a, b) => a - b);
//   if (algorithm === "insertionSort") return [...inputArr].sort((a, b) => a - b);
...
function computeOutput(algorithm, inputArr) {
  ...
  // Only array-type algorithms have an "Output array"
...
export default function Simulator() {
  ...
  // graph input state + default graph
  const [algoType, setAlgoType] = useState("array"); // "array" | "graph"

  useEffect(() => {
    const meta = ALGORITHMS[algorithm];
    setAlgoType(meta?.type || "array");
    // also clear steps/logs when switching algorithms
    setSteps([]);
    setStepIndex(0);
    setStatus("idle");
  ...
  const navActionRef = useRef("init");          // "next" | "auto" | "prev" | "seek" | "init"
  const loggedStepsRef = useRef(new Set());     // step indices already logged
  ...
  // when steps/step changes, if mentor mode and question exists -> lock
  useEffect(() => {
    if (!mentorMode) {
    ...
    if (q) {
      ...
      setMentorFeedback(null);
      // stop autoplay if it was playing
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      ...
      setStepIndex((idx) => {
        const next = Math.min(idx + 1, maxStep);
        return next;
      });
  ...
  const onBuildSteps = () => {
    ...
    // Validate input based on algo type
    if (meta.type === "array") {
    ...
    // Build steps
    const built =
      meta.type === "array" ? meta.build(inputArr)
        : meta.type === "graph" ? meta.build(graphInput)
          : meta.build(dpInput);
  ...
  const onReset = () => {
    setStepIndex(0);
    setStatus("idle");
    setMentorFeedback(null);    // if mentor mode and first step has question, lock will be set by effect
  ...
  const goNext = () => {
    if (!steps.length) return;
    if (mentorMode && mentorLocked) return; // locked
  ...
  // keyboard: Right arrow = next (manual)
  useEffect(() => {
    const onKey = (e) => {
  ...
  return (
    <div className="min-h-screen bg-zinc-100 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-50">
      ...
      {/* Split: Left Visualization, Right Code */}
      <div className="max-w-[1400px] mx-auto px-4 mt-4">
          ...
          <ResizableSplit
            ...
            left={
              <div className="h-full">
                ...
                {/* <Visualizer arr={cur?.array || inputArr} comparing={cur?.comparing} swapped={cur?.swapped} /> */}
